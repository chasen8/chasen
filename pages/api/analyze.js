...
export default async function handler(req, res) {
  ...
  if (!symbol || !interval) {
    return res.status(400).json({ error: '請提供 symbol 和 interval' });
  }

  const limit = 500;
  const klinesUrl = `https://fapi.binance.com/fapi/v1/klines?symbol=${symbol}&interval=${interval}&limit=${limit}`;

  try {
    const response = await fetch(klinesUrl);
    if (!response.ok) {
      return res.status(500).json({ error: '無法取得 Binance K 線資料' });
    }
    ...
    const sortedZones = zoneVolume.map((vol, idx) => ({ idx, vol }))
    ...
    let advice = '觀望';
    if (latestMACD.histogram > 0 && latestRSI < 70) advice = '建議做多';
    else if (latestMACD.histogram < 0 && latestRSI > 30) advice = '建議做空';
